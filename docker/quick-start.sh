...
# Gemini API 配置（可选，不设置也可以启动）
# GEMINI_API_KEY=你的_GEMINI_API_KEY

# OpenAI API 配置（可选，不设置也可以启动）
...
LANG=C.UTF-8
EOF
        log_warning "注意：即使不设置 API 密钥，应用也可以正常启动"
        log_info "如需使用 AI 功能，请编辑 .env 文件并填入相应的 API 密钥"
    else
...
main() {
    ...
    log_info "🎉 恭喜！应用已成功启动"
    log_info "📍 访问地址: http://localhost:8501"
    log_info "📝 注意：即使没有 API 密钥，应用也可以正常启动和使用"
    log_info "🔑 如需 AI 功能，请编辑 .env 文件设置相应的 API 密钥"
    echo ""
