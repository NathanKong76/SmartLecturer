...
PACKAGE_NAME="lecturer-macos-v$VERSION"
...
if [ -d "$DIST_DIR/lecturer.app" ]; then
    cp -r "$DIST_DIR/lecturer.app" "$PACKAGE_DIR/"
    info "已复制 .app 包"
fi
...
#!/bin/bash
cd "$(dirname "$0")"
if [ -f "./lecturer.app/Contents/MacOS/lecturer" ]; then
    ./lecturer.app/Contents/MacOS/lecturer
elif [ -f "./lecturer" ]; then
    ./lecturer
else
    echo "错误: 未找到可执行文件"
    exit 1
...
if [ -d "$PACKAGE_DIR/lecturer.app" ]; then
    info "应用程序包: $PACKAGE_DIR/lecturer.app"
fi
if [ -f "$PACKAGE_DIR/lecturer" ]; then
